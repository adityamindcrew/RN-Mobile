...
import { BOOKINGS, OTHER_USER_PROFILE } from "../../navigation/RouteNames";
...
const CurrentBookin = ({ navigation, route }) => {
  ...
  const [guestFees, setGuestFees] = useState("");
    ...
    if (data.length !== 0) {
      setAllImages(
        data[0].listingId.images.length !== 0
          ? data[0].listingId.images.sort((a, b) => a.position - b.position)
          : [{ uri: STOCK_PHOTO }]
      );
      ...
      const newPricing = data[0].guestFees * diff;

      setCurrencyPrice(data[0].amount / diff);
      setGuestFees(data[0].guestFees || 0);
      var totalAmount =
        data[0].amount +
  ...
  const updateReservationStatus = async (response) => {
    ...
    if (data[0].listingId.instantBookingAllowed) {
      ...
      try {
        ...
        navigation.goBack();
        setLoading(false);
        Alert.alert("Cercles", "Your reservation has been cancelled");
      } catch (error) {
        console.log(error);
        setLoading(false);
              ...
              <View style={styles.priceContainer}>
                ...
                <View style={styles.priceView}>
                  <Typography.ChileLine
                    ...
                    fontweight={500}
                  >
                    -{currencyType.symbol + priceFormat(data[0].promotion || 0)}
                  </Typography.ChileLine>
                </View>
              </View>
            ) : (
